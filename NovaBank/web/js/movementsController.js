...
async function createNewMovement(e) {
    ...
    try {
        ...
        // PROBLEMA 2: Validación por Expresión Regular
        const amountRegex = /^\d{1,3}(\.\d{3})*(,\d{2})?$/;
        if (!amountRegex.test(rawAmount)) {
            throw new Error("Formato de monto inválido. Use el formato 1.234,56");
        }

        // PROBLEMA 2: Formateo de valor para operativa CRUD
        // 1. Quitamos los puntos de miles | 2. Cambiamos la coma decimal por punto
        ...
        // Lógica de saldo (simplificada para el ejemplo)
        ...
        msgBox.textContent = error.message; // Mensajes específicos (Problema 1)
